...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        graphicsStroke = new BasicStroke(5.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 10.0f,
                new float[] { 30, 10, 20, 10, 10, 10, 20, 10 }, 0.0f); // 24,6,12,6,6,6,12,6
    ...
    private void paintLabels(Graphics2D canvas) {
        ...
        if (selectedMarker >= 0) {
            ...
            String label = "X=" + formatter.format(((Double[]) graphicsData.get(selectedMarker))[0]) + ", Y="
    ...
    protected void paintGraphics(Graphics2D canvas) {
        ...
        for (Double[] point : this.graphicsData) {
            if ((point[0].doubleValue() >= this.viewport[0][0]) && (point[1].doubleValue() <= this.viewport[0][1])
                    && (point[0].doubleValue() <= this.viewport[1][0])
                    && (point[1].doubleValue() >= this.viewport[1][1])) {
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (int i = 0; i < graphicsData.size(); i++) {
            ...
            if (i != 0 && i != graphicsData.size() - 1
                    && ((graphicsData.get(i - 1)[1] < graphicsData.get(i)[1]
                    && graphicsData.get(i)[1] > graphicsData.get(i + 1)[1])
                    || (graphicsData.get(i - 1)[1] > graphicsData.get(i)[1]
                    && graphicsData.get(i)[1] < graphicsData.get(i + 1)[1]))) {
            ...
            if (flag == false) {
                ...
                canvas.drawString(
                        "(" + tempX.format(graphicsData.get(i)[0]) + "; " + tempY.format(graphicsData.get(i)[1]) + ")",
                        (float) labelPos.getX() + 5, (float) (labelPos.getY() - bounds.getY()) - 20);
            }
        }
    }
    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        if(!(viewport[1][1]>0.0D||viewport[0][1]<0.0D))
        {
            ...
            canvas.drawString("x", (float) (labelPos.x - bounds.getWidth() - 10),
                    (float) (labelPos.y - bounds.getHeight() / 2));
        }
    }
        ...
        public void mouseClicked(MouseEvent ev) {
            if (ev.getButton() == 3) {
                if (GraphicsDisplay.this.undoHistory.size() > 0) {
                    GraphicsDisplay.this.viewport = (double[][]) GraphicsDisplay.this.undoHistory
                            .get(GraphicsDisplay.this.undoHistory.size() - 1);
            ...
            if (ev.getButton() == 1) {
                ...
                if (GraphicsDisplay.this.changeMode) {
                    ...
                    GraphicsDisplay.this.zoomToRegion(GraphicsDisplay.this.originalPoint[0],
                            GraphicsDisplay.this.originalPoint[1], finalPoint[0], finalPoint[1]);
                    GraphicsDisplay.this.repaint();
                }
    ...
    public class MouseMotionHandler implements MouseMotionListener {
        ...
        public void mouseDragged(MouseEvent ev) {
            if (GraphicsDisplay.this.changeMode) {
                ...
                double newY = ((Double[]) GraphicsDisplay.this.graphicsData.get(GraphicsDisplay.this.selectedMarker))[1]
                        + (currentPoint[1] - ((Double[]) GraphicsDisplay.this.graphicsData
                        .get(GraphicsDisplay.this.selectedMarker))[1]);
